...
// esp_err_t jitter_buffer_read(jitter_buffer_handle_t handle, uint8_t *data, size_t len);
// /** 内部 API：不带头部的原始环形写入（不打包，不解析帧） */
// esp_err_t jitter_buffer_write_internal(jitter_buffer_handle_t handle, const uint8_t *data, size_t len);
// /** 内部 API：不带头部的原始环形读取，返回实际读到的字节数，通过 out_len 返回 */
// esp_err_t jitter_buffer_read_internal(jitter_buffer_handle_t handle, uint8_t *data, size_t len, size_t *out_len);
